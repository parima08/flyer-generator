...
app.service('articleDetailsService', function ($http, $q, $sce) {
				...
				fetch(sheetDataUrl).then(response => response.json())
					.then(data => {
						...
						for (let i = 1; i < values.length; i++) {
							let article = values[i];
							//console.log(article);
							let articleInfo = grabArticleInfo(article);
			...
			.then(title => {
				...
				fetch(sheetDataUrl).then(response => response.json())
					.then(data => {
						...
						articleDetails = articlesData.filter(a => a.name.trim() === name.trim())[0];
	...
	var grabArticleInfo = function (article) {
		...
		return {
			...
			year: article[2],
			wordCount: article[3],
			magazineSection: article[4],
			category: article[5],
			language: article[6],
			articleLink: article[7],
			documentLink: article[8]
		}
	}
